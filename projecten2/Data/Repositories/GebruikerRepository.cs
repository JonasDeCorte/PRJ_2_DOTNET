...
namespace projecten2.Data.Repositories
{
    public class GebruikerRepository : IGebruikerRepository
    {
        ...
        {
            return _gebruikers.Include(x => x.Contracten).ThenInclude(x => x.ContractType).Include(x => x.Tickets).ThenInclude(x => x.TicketType).FirstOrDefault(x => x.Email == email);
        }
        #endregion
        ...
        {
            return _contracten.Include(x => x.ContractType).Include(x => x.Tickets).ThenInclude(x => x.TicketType).ToList();
        }

        public Contract GetByContractNr(int contractNr)
        {
            return _contracten.Include(x => x.ContractType).Include(x => x.Tickets).ThenInclude(x => x.TicketType).FirstOrDefault(x => x.ContractNr == contractNr);
        }
